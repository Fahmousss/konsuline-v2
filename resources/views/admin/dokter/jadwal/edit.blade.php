...
@push('scripts')
    <script>
        window.doctorScheduleData = @json($doctor->doctorSchedule);
        window.hariOptionsData = @json(array_map(fn($h) => $h->value, \App\Enums\Hari::cases()));
    </script>
    <script>
        function jadwalManager(initialJadwals = [], doctorId, hariOptions = [
            ...
            return {
                ...
                async simpanJadwal() {
                    this.createErrors = [];
                    try {
                        const res = await fetch(`/admin/dokter/${doctorId}/jadwal`, {
                            method: 'POST',
                            headers: {
                ...
                showEdit(jadwal) {
                    ...
                    // Hapus aria-hidden yang mungkin tertinggal
                    modalEl.removeAttribute('aria-hidden');

                    modal.show();

                    // Fokus ke input pertama agar tidak ke tombol close
                    setTimeout(() => {
                ...
                async updateJadwal() {
                    this.editErrors = [];
                    try {
                        const res = await fetch(`/admin/dokter/jadwal/${this.editForm.id}`, {
                            method: 'PUT',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                ...
                async hapusJadwal(id) {
                    ...
                    try {
                        const res = await fetch(`/admin/dokter/jadwal/${id}`, {
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                                'Accept': 'application/json',
...
<x-admin-layout :title="'Edit Jadwal - ' . $doctor->user->name">
    <div class="container mt-4" x-data="jadwalManager(window.doctorScheduleData, {{ $doctor->id }}, window.hariOptionsData)">
        <h2 class="mb-3">Tambah Jadwal</h2>
        <div x-show="successMessage" class="alert alert-success" x-text="successMessage"></div>
        <div x-show="createErrors.length" class="alert alert-danger" x-html="createErrors.join('<br>')">
        </div>
        ...
        <template x-if="jadwals.length === 0">
            <div class="alert alert-info">Belum ada jadwal untuk dokter ini.</div>
        </template>
                ...
                <tbody>
                    <template x-for="jadwal in jadwals" :key="jadwal.id">
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-sm btn-warning" @click="showEdit(jadwal)">
                                    Edit
                                </button>
                                <button class="btn btn-sm btn-danger" @click="hapusJadwal(jadwal.id)">
                                    Hapus
                                </button>
                            </td>
                        </tr>
